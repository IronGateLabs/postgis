...
- **[Trade-off] Host-side ERA precomputation serializes the hot path** -- Fixing the rotate_z_m_epoch precision bug required moving ERA computation from the GPU kernel (which ran in parallel across all threads) to the host (which runs as a serial scalar loop). Benchmarks on A18 Pro show the corrected Metal path at ~70 Mpts/s for 500K points, compared to NEON at ~76 Mpts/s — roughly a wash, vs. the 2.4x Metal win that the previous (incorrect, ~900 km error) path reported. This means Metal is no longer a clear throughput winner for rotate_z_m_epoch at the currently-tested workloads. **Mitigation (future work, not in this change):** parallelize the host-side ERA precomputation using NEON/AVX SIMD. The existing NEON rotate_z_m_epoch implementation in `liblwgeom/accel/rotate_z_neon.c` already computes ERA in 2-wide `float64x2_t` SIMD; a refactored helper could precompute just the thetas array (half the work NEON already does) and feed it to the Metal kernel. Estimated 1.5–2x speedup over the current Metal path, which would restore Metal as a clear win at large point counts. Tracking as a future OpenSpec change.

- **[Note] Dispatch decision unchanged by the correctness fix** -- The `effective_gpu_threshold()` and operation-based routing in `lwgeom_accel.c` still dispatch Metal for `rotate_z_m_epoch` at 50K+ points. The 8% performance gap vs NEON at 500K is within measurement noise and device-variation range, and Metal may still win at larger workloads or on higher-core-count Apple GPUs (M3 Max, M4 Max). Users who prefer to avoid the precision tradeoff entirely can set `postgis.gpu_dispatch_threshold = 0` to disable GPU dispatch and rely on NEON.

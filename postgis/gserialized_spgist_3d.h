...
/*
 * Compact the traversal results after inner-consistent node filtering.
 * Copies the selected nodeNumbers and traversalValues into palloc'd arrays
 * sized to out->nNodes, then frees the temporary over-sized buffers.
 */
static inline void
spgist_compact_traversal(spgInnerConsistentOut *out, int *nodeNumbers, void **traversalValues)
{
	int i;
